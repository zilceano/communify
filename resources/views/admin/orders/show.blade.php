<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            @if($order->status == 'draft')
                <form action="{{ route('order.finalize', $order->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                            ...
                            <div class="bg-white p-6 shadow sm:rounded-lg">
                                ...
                                @foreach($order->items as $item)
                                    <div class="flex items-center border-b border-gray-100 pb-4 mb-4">
                                        <img src="{{ asset('storage/' . $item->product->image_mockup) }}" class="w-16 h-16 rounded object-cover mr-4">
                                        <div>
                                            ...
                                            <p class="text-sm text-gray-500">
                                                ...
                                                @if($item->selected_options_json)
                                                    ...
                                                    @foreach($item->selected_options_json as $key => $val)
                                                        <span class="bg-gray-100 px-1 rounded text-xs">{{ $key }}: {{ $val }}</span>
                                                    @endforeach
                                                @endif
                                            </p>
                                        </div>
                                        <div class="ml-auto font-bold">R$ {{ number_format($item->price, 2, ',', '.') }}</div>
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="space-y-6">
                            <div class="bg-white p-6 shadow sm:rounded-lg" x-data="{ donation: 0, shipping: 30.00, subtotal: {{ $order->items->sum('price') }} }">
                                ...
                                <div class="space-y-2 text-sm border-t pt-4">
                                    <div class="flex justify-between">
                                        ...
                                        <span class="font-bold">R$ {{ number_format($order->items->sum('price'), 2, ',', '.') }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                ...
                                <button type="submit" class="w-full mt-6 bg-green-600 text-white font-bold py-3 rounded-lg hover:bg-green-700 shadow-lg">
                                    Finalizar e Pagar
                                </button>
            ...
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                        <h3 class="text-lg font-bold text-gray-900 mb-4">Resumo da Compra</h3>
                            ...
                            @foreach($order->items as $item)
                                <div class="flex items-start">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500">{{ $item->product->baseProduct->name }}</p>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                        ...
                        @if($order->status == 'awaiting_payment')
                            @if(!$order->proof_of_payment)
                                <div class="bg-gray-100 border-2 border-dashed border-gray-300 rounded-lg p-6 text-center mb-6">
                                    ...
                                    <p class="text-xs text-indigo-600 mt-2">Valor exato: R$ {{ number_format($order->total_amount, 2, ',', '.') }}</p>
                                </div>
